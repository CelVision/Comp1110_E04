...
int main() {
    ...
    if (!nodeFile.is_open()) {
        cerr << "Error: Cannot open node.txt. Please ensure it is in the same folder." << endl;
        return 1;
    }
    ...
    ifstream neighborFile("neighbor.txt");

    // Check if neighbor.txt can be opened
    if (!neighborFile.is_open()) {
        cerr << "Error: Cannot open neighbor.txt. Please ensure it is in the same folder." << endl;
        return 1;
    }
